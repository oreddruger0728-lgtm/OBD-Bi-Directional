...
import { WiFiTransport } from "@/utils/wifiTransport";
...
export function OBD2Provider({ children }: { children: React.ReactNode }) {
  ...
  // ── WiFi polling loop (identical to BT but uses wifiTransportRef) ───────────
  const startWiFiPolling = useCallback(async () => {
    const transport = wifiTransportRef.current;
    if (!transport) return;

    btPollActiveRef.current = true; // reuse same flag
      ...
      while (btPollActiveRef.current && transport.isConnected()) {
        for (const pid of LIVE_PIDS) {
          ...
          try {
            ...
            // PID not supported or timeout — skip
          }
        }
        await new Promise((r) => setTimeout(r, 100));
      }
  ...
  const connect = useCallback(async (): Promise<boolean> => {
    ...
    if (connectionMode === "WIFI") {
      try {
        // Clean up any previous WiFi transport
        btPollActiveRef.current = false;
        wifiTransportRef.current?.disconnect().catch(() => {});
        wifiTransportRef.current = null;

        const transport = new WiFiTransport();
        await transport.connect(wifiHost, wifiPort, 8000);
        wifiTransportRef.current = transport;

        // Run identical ELM327 init sequence
        for (let i = 0; i < ELM327_INIT_COMMANDS.length; i++) {
          const { cmd, delayMs } = ELM327_INIT_COMMANDS[i];
          ...
          try {
            ...
            // Non-fatal — older adapters may not support some AT commands
          }
          await new Promise((r) => setTimeout(r, delayMs));
        }
        ...
        wifiTransportRef.current = null;
        setConnectionStatus("ERROR");
        console.warn("[WiFi Connect Error]", e?.message ?? String(e));
        // Common failures:
        // "WiFi connect timeout" → wrong IP/port, phone not on adapter hotspot
        // "Network request failed" → phone not connected to adapter WiFi network
        // "Connection refused" → adapter powered off or wrong port
        return false;
      }
    }
    ...
    setConnectionStatus("ERROR");
    return false;
  }, [connectionMode, btDeviceAddress, btAvailable, wifiHost, wifiPort, startDemoPolling, startBTPolling, startWiFiPolling]);
  ...
  const sendRawCommand = useCallback(async (cmd: string, timeoutMs = 3000): Promise<string> => {
    ...
    if (connectionMode === "WIFI" && wifiTransportRef.current) {
      return wifiTransportRef.current.send(cmd, timeoutMs);
    }
  ...
  const runServiceReset = useCallback(async (resetId: string): Promise<{ success: boolean; message: string }> => {
    ...
    const activeTransport = connectionMode === "BT" ? btTransportRef.current :
      connectionMode === "WIFI" ? wifiTransportRef.current : null;

    if (activeTransport) {
      const log: string[] = [];
      try {
        for (const step of reset.obdCommands) {
          const raw = await activeTransport
        ...
        activeTransport?.send("ATSH 7E0", 500).catch(() => {});
        activeTransport?.send("1001", 500).catch(() => {});
      }
    }
